...
public class ShrivingForgeMenu extends AbstractContainerMenu {
    ...
    public ShrivingForgeMenu(int containerId, Inventory inv, FriendlyByteBuf extraData) {
        this(containerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(2));
    }

    public ShrivingForgeMenu(int containerId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(utsMenuRegistry.SHRIVING_FORGE_MENU.get(), containerId);

        checkContainerSize(inv, 3);

        blockEntity = (ShrivingForgeTile) entity;
        ...
        slot1 = new SlotItemHandler(itemHandler, 0, 62, 30) {
            @Override
        ...
        slot2 = new SlotItemHandler(itemHandler, 1, 98, 30) {
            @Override
            public boolean mayPlace(ItemStack stack) {
        ...
        slot3 = new SlotItemHandler(itemHandler, 2, 80, 50) {
            @Override
    ...
    public float getProgress() {
        ...
        return maxProgress > 0
                ? Math.min(1.0F, progress / (float) maxProgress)
                : 0.0F;
    }
    ...
    public boolean canTakeItemForPickAll(ItemStack pStack, Slot pSlot) {
        return pSlot.container != this.slot3.container && super.canTakeItemForPickAll(pStack, pSlot);
    }

    @Override
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                pPlayer, SHRIVING_FORGE_BLOCK.get());
    }
