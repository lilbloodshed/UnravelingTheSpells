...
public final class UTSCommand {
    ...
    private static Set<ResourceLocation> learnedIds(ServerPlayer player, KnowledgeType type) {
        return type == KnowledgeType.SPELL
                ? PlayerSpellProvider.get(player).getSpells()
                : PlayerSchoolProvider.get(player).getSchools();
    }
    ...
    private static int showLearned(
            ...
            ServerPlayer player,
            KnowledgeType type) {
        Set<ResourceLocation> displayed = new LinkedHashSet<>(learnedIds(player, type));
    ...
    private static Stream<ResourceLocation> allIds(KnowledgeType type) {
        ...
        return SchoolRegistry.REGISTRY.stream().map(school -> school.getId());
    }
