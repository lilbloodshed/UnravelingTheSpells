    ...
    public class LearningScreen extends AbstractContainerScreen<MagicLecternMenu> {
        ...
        public LearningScreen(MagicLecternMenu menu, Inventory playerInventory, Component title) {
        ...
        private void openList() {
            ...
            player.getCapability(PlayerSchoolProvider.PLAYER_SCHOOL).ifPresent(schoolData -> {
                player.getCapability(PlayerSpellProvider.PLAYER_SPELL).ifPresent(spellData -> {
                    if (isInitialized) {
                        updateUI(schoolData, spellData);
                    } else {
                        ModMessages.sendToServer(new RequestSyncPacket());
        ...
        private synchronized void updateUI(PlayerSchool schoolData, PlayerSpell spellData) {
            ...
            selectedSchools.addAll(schoolData.getSchools());

            learnedSpells.clear();
            learnedSpells.addAll(spellData.getSpells());
