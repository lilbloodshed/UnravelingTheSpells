...
public class SchoolCommand {
    ...
    private static int learnSchool(CommandSourceStack source, ResourceLocation schoolId, Player player) {
        ...
        if (schoolData.getSchools().size() >= Configuration.getMaxSchools()) {
            source.sendFailure(Component.literal("Player already has maximum number of schools (" + Configuration.getMaxSchools() + ")!")
